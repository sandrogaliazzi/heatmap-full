...
### Requirement: Cache client-side no store tomodat (usuário autenticado)
...
**Migration**: Remover do arquivo `frontend/src/stores/tomodat.js`:
- Funções: `readCache`, `writeCache`, `invalidateCache`
- Constantes: `CACHE_KEY_FULL`, `CACHE_KEY_GUEST`, `CACHE_TTL_MS`
- Na função `getTomodatData`: remover a verificação de cache (leitura e escrita de localStorage) para o fluxo do usuário autenticado. A função passa a chamar `GET /newfetch` e `GET /cables` diretamente a cada invocação.
